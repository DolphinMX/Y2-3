...
void retain(node<Type>*& listA, const node<Type> *listB)
{
	...
	node<Type> *p = listA;
	node<Type> *r = nullptr;  // r points to the predecessor of p

	const node<Type> *q = listB;

	while (p != nullptr && q != nullptr)
	{
		if (p->info == q->info)  // retain node p
		{
			r = p;
			p = p->link;
			q = q->link;
	...
	// assert: node p and its successors are not found in listB
	if (p == listA)
	{
		while (p != nullptr)
		{
	...
	else // assert: r != nullptr, delete all successors of r
		while (p != nullptr)
		{
			r->link = p->link;
			delete p;
	...
	/* Alternative design of the second while-loop
	// Remove node p and its successors
	while (p != nullptr)
	{
		if (p == listA)
		{
	...
	*/
	...
	/* Another program organization

	while (p != nullptr)
	{
		if (q == nullptr || p->info < q->info)
			// delete node p
		else if (p->info > q->info)
			// advance q
		else
			// retain node p, advance r, p, and q
	}
	*/
}
...
void retain_2(dnode<Type>*& listA, const dnode<Type> *listB)
{
	...
	while (p != nullptr && q != nullptr)
	{
		...
		else // remove node p from listA
		{
			...
			{
				dnode<Type> *r = p->back;  // r points to predecessor of p
				r->next = p->next;
				if (p->next != nullptr)
					p->next->back = r;
				delete p;
