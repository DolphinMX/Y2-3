...
void updateCount_binSearch(freqCount *countList, int& n, const string& word)
{
	...
	{
		int mid = (low + high) / 2;
		int c = word.compare(countList[mid].word);
		if (c == 0)
			loc = mid;
		else if (c > 0)
			low = mid + 1;
	...
	if (loc >= 0)	
		countList[loc].count++;
	else  // insert word at index low (point of insertion)
	{
		for (int i = n-1; i >= low; i--)
			countList[i+1] = countList[i];

		countList[low].word = word;
		countList[low].count = 1;
		n++;
	}
}
...
void updatePopList(freqCount *popList, int& k, int rankThreshold, const freqCount& item)
{
	...
	if (k >= rankThreshold && item.count < popList[k-1].count)
		return;

	int j;
	for (j = k-1; j >= 0 && popList[j].count < item.count; j--)	
		popList[j+1] = popList[j];
	
	popList[j+1] = item;
	k++;

	if (k > rankThreshold && 
		popList[rankThreshold-1].count > popList[rankThreshold].count)
		k = rankThreshold;  
		// Discard items ranked lower than rankThreshold if they have smaller count value.
}
...
void printCountList(const freqCount *countList, int t)
{
	cout << "\nFirst " << t << " records in countList:\n";
	for (int i = 0; i < t; i++)
...
{	
	...
	// print parts of countList for development or debugging purpose
	printCountList(countList, 30);
